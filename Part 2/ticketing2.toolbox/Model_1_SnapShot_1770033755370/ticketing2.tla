...
(* --algorithm ticketing {
    ...
    define {
        ...
        ClientReceivesWhatTheyPayFor ==
            \A c \in AllHonest :
        (BankAccount[c] < INITMONEY) => (Cardinality(Tickets[c]) > 0)
        \* -----------------------------
        \* Stop condition
        \* -----------------------------
        AllDone ==
    ...
    variables
        ...
        targetSeat = 1;
        reply = M0;
        scamsCount = 0; \* Track number of attempts
    {
        MStep:
        \* Limit the malicious client to 2-3 scams so the model can terminate
        while (scamsCount < 2 /\ ~AllDone) { 
...
\* BEGIN TRANSLATION (chksum(pcal) = "a6abbe0" /\ chksum(tla) = "3b80a59b")
\* Label s1 of process Server at line 90 col 13 changed to s1_
\* Label Done_ of process Server at line 137 col 9 changed to Done__
\* Process variable id of process Server at line 86 col 9 changed to id_
\* Process variable ip of process Server at line 87 col 9 changed to ip_
\* Process variable ip of process HClient at line 143 col 9 changed to ip_H
\* Process variable reply of process HClient at line 146 col 9 changed to reply_
VARIABLES BankAccount, Channels, seatMap, Tickets, CState, pc
...
vars == << BankAccount, Channels, seatMap, Tickets, CState, pc, id_, ip_, 
           internalReq, id, ip_H, wantSeat, reply_, target, availSeats, ip, 
...
s1_ == /\ pc[0] = "s1_"
       ...
       /\ UNCHANGED << BankAccount, Channels, seatMap, Tickets, CState, id_, 
                       ip_, internalReq, id, ip_H, wantSeat, reply_, target, 
                       availSeats, ip, targetID, targetSeat, reply, scamsCount >>
...
GET == /\ pc[0] = "GET"
       ...
       /\ UNCHANGED << BankAccount, seatMap, Tickets, CState, id_, ip_, id, 
                       ip_H, wantSeat, reply_, target, availSeats, ip, 
                       targetID, targetSeat, reply, scamsCount >>
...
TREAT == /\ pc[0] = "TREAT"
         ...
         /\ pc' = [pc EXCEPT ![0] = "s1_"]
         /\ UNCHANGED << CState, id_, ip_, internalReq, id, ip_H, wantSeat, 
...
InitTarget(self) == /\ pc[self] = "InitTarget"
                    ...
                    /\ UNCHANGED << BankAccount, Channels, seatMap, Tickets, 
                                    ...
                                    wantSeat, reply_, availSeats, ip, targetID, 
...
s1(self) == /\ pc[self] = "s1"
            ...
            /\ UNCHANGED << BankAccount, Channels, seatMap, Tickets, CState, 
                            id_, ip_, internalReq, id, ip_H, wantSeat, reply_, 
                            target, availSeats, ip, targetID, targetSeat, 
                            reply, scamsCount >>
...
CheckDone(self) == /\ pc[self] = "CheckDone"
                   /\ IF Cardinality(Tickets[self]) >= target[self]
                         ...
                         ELSE /\ pc' = [pc EXCEPT ![self] = "BWaitIdle"]
                              /\ UNCHANGED CState
                   /\ UNCHANGED << BankAccount, Channels, seatMap, Tickets, 
                                   id_, ip_, internalReq, id, ip_H, wantSeat, 
                                   reply_, target, availSeats, ip, targetID, 
                                   targetSeat, reply, scamsCount >>
...
BSend(self) == /\ pc[self] = "BSend"
               ...
               /\ UNCHANGED << BankAccount, seatMap, Tickets, id_, ip_, 
                               internalReq, id, ip_H, reply_, target, ip, 
                               targetID, targetSeat, reply, scamsCount >>
...
BWaitReply(self) == /\ pc[self] = "BWaitReply"
                    ...
                    /\ UNCHANGED << BankAccount, seatMap, Tickets, CState, id_, 
                                    ip_, internalReq, id, ip_H, wantSeat, 
                                    target, availSeats, ip, targetID, 
                                    targetSeat, reply, scamsCount >>
...
BUpdate(self) == /\ pc[self] = "BUpdate"
                 ...
                 /\ pc' = [pc EXCEPT ![self] = "s1"]
                 /\ UNCHANGED << BankAccount, Channels, seatMap, Tickets, id_, 
                                 ip_, internalReq, id, ip_H, wantSeat, reply_, 
                                 target, availSeats, ip, targetID, targetSeat, 
                                 reply, scamsCount >>
...
MStep(self) == /\ pc[self] = "MStep"
               ...
               /\ UNCHANGED << BankAccount, Channels, seatMap, Tickets, CState, 
                               id_, ip_, internalReq, id, ip_H, wantSeat, 
                               reply_, target, availSeats, ip, reply, 
                               scamsCount >>
...
MSend(self) == /\ pc[self] = "MSend"
               ...
               /\ UNCHANGED << BankAccount, seatMap, Tickets, CState, id_, ip_, 
