...
(* --algorithm ticketing {
    variables
        ...
        ipOf        = [c \in AllHonest |-> c]; \* current IP address for each honest client (server replies to ipOf[bankID])
    ...
    define {
        ...
        \* protocol messages
        TransactionType ==
          {"list", "listResp", "reserve", "pay", "cancel", "updateIP", "confirm", "deny"}
        ...
        MessageType == [ type     : TransactionType,
                         ...
                         bankID   : bankIDType,
                         password : Nat,
                         seats    : SUBSET Seats ] \*field 'seats' used only in listResp (empty otherwise)
        ...
        internalReq = M0;
    {
        s1: while (~AllDone) {
            ...
            \* list: return available seats
            } else if (internalReq.type = "list") {
                if (internalReq.bankID \in AllHonest
                    /\ internalReq.password = Password[internalReq.bankID]) {

                    Channels[ipOf[internalReq.bankID]] :=
                      Append(Channels[ipOf[internalReq.bankID]],
                             [type |-> "listResp", from |-> 0, seat |-> 1,
                              bankID |-> -2, password |-> 0,
                              seats |-> {s \in Seats : seatMap[s] = "available"}]);
                } else {
        ...
        s1: while (CState[self] # "done") {
            ...
            if (Cardinality(Tickets[self]) >= target
                ...
                either {
                    ...
                    \* step 1: ask for available seats
                    LWaitIdle:
                    await (CState[self] = "idle");
                    ...
                    \* step 2: reserve one of the returned seats (if any)
                    RSend:
                    availSeats := reply.seats;
        ...
        scamsCount = 0;
    {
        ...
        while (scamsCount < 1 /\ ~AllDone) {
            ...
            \* try to reserve using targetID but attacker password -> should fail
            MSend:
            Channels[0] := Append(Channels[0],
...
\* BEGIN TRANSLATION (chksum(pcal) = "ebe3abf5" /\ chksum(tla) = "9650a940")
\* Label s1 of process Server at line 108 col 13 changed to s1_
\* Label Done_ of process Server at line 234 col 9 changed to Done__
\* Label Done_ of process HClient at line 347 col 9 changed to Done__H
\* Process variable ip of process Server at line 105 col 9 changed to ip_
\* Process variable ip of process HClient at line 240 col 9 changed to ip_H
\* Process variable reply of process HClient at line 243 col 9 changed to reply_
...
s1_ == /\ pc[0] = "s1_"
       ...
       /\ UNCHANGED << BankAccount, Channels, seatMap, ResOwner, Tickets, 
                       ...
                       wantSeat, reply_, target, reservedSeat, availSeats, 
                       doCancel, ip, targetID, targetSeat, reply, scamsCount >>
...
GET == /\ pc[0] = "GET"
       ...
       /\ UNCHANGED << BankAccount, seatMap, ResOwner, Tickets, CState, 
                       ...
                       target, reservedSeat, availSeats, doCancel, ip, 
                                                ...
                                                THEN /\ seatMap' = [seatMap EXCEPT ![internalReq.seat] = "reserved"]
                                                     ...
                                                     /\ Channels' = [Channels EXCEPT ![ipOf[internalReq.bankID]] = Append(Channels[ipOf[internalReq.bankID]],
                                                                                                                          [type |-> "confirm", from |-> 0,
                                                                                                                           seat |-> internalReq.seat,
                                                                                                                           bankID |-> -2, password |-> 0, seats |-> {}])]
                                                ELSE /\ Channels' = [Channels EXCEPT ![ipOf[internalReq.bankID]] = Append(Channels[ipOf[internalReq.bankID]],
                                                                                                                          [type |-> "deny", from |-> 0,
                                                                                                                           seat |-> internalReq.seat,
                                                                                                                           bankID |-> -2, password |-> 0, seats |-> {}])]
                                                     /\ UNCHANGED << seatMap, 
...
InitTarget(self) == /\ pc[self] = "InitTarget"
                    ...
                    /\ UNCHANGED << BankAccount, Channels, seatMap, ResOwner, 
                                    ...
                                    reservedSeat, availSeats, doCancel, ip, 
                                    targetID, targetSeat, reply, scamsCount >>
...
s1(self) == /\ pc[self] = "s1"
            ...
            /\ UNCHANGED << BankAccount, Channels, seatMap, ResOwner, Tickets, 
                            ...
                            availSeats, doCancel, ip, targetID, targetSeat, 
                            reply, scamsCount >>
...
ChWaitIdle(self) == /\ pc[self] = "ChWaitIdle"
                    ...
                    /\ UNCHANGED << BankAccount, Channels, seatMap, ResOwner, 
                                    Tickets, CState, MyTickets, ipOf, ip_, 
                                    internalReq, id, ip_H, wantSeat, reply_, 
                                    target, reservedSeat, availSeats, doCancel, 
...
ChWait(self) == /\ pc[self] = "ChWait"
                ...
                /\ UNCHANGED << BankAccount, seatMap, ResOwner, Tickets, 
                                CState, MyTickets, ipOf, ip_, internalReq, id, 
                                ip_H, wantSeat, target, reservedSeat, 
                                availSeats, doCancel, ip, targetID, targetSeat, 
                                reply, scamsCount >>
...
LWaitIdle(self) == /\ pc[self] = "LWaitIdle"
                   /\ (CState[self] = "idle")
                   /\ pc' = [pc EXCEPT ![self] = "LSend"]
                   /\ UNCHANGED << BankAccount, Channels, seatMap, ResOwner, 
                                   Tickets, CState, MyTickets, ipOf, ip_, 
                                   internalReq, id, ip_H, wantSeat, reply_, 
                                   target, reservedSeat, availSeats, doCancel, 
...
LSend(self) == /\ pc[self] = "LSend"
               /\ CState' = [CState EXCEPT ![self] = "waiting"]
               /\ Channels' = [Channels EXCEPT ![0] =            Append(Channels[0],
                                                      [type |-> "list", from |-> ip_H[self], seat |-> 1,
                                                       bankID |-> id[self], password |-> Password[id[self]], seats |-> {}])]
...
LWait(self) == /\ pc[self] = "LWait"
               ...
               /\ reply_' = [reply_ EXCEPT ![self] = Head(Channels[ip_H[self]])]
               /\ Channels' = [Channels EXCEPT ![ip_H[self]] = Tail(Channels[ip_H[self]])]
               /\ pc' = [pc EXCEPT ![self] = "RSend"]
               /\ UNCHANGED << BankAccount, seatMap, ResOwner, Tickets, CState, 
                               MyTickets, ipOf, ip_, internalReq, id, ip_H, 
                               wantSeat, target, reservedSeat, availSeats, 
                               doCancel, ip, targetID, targetSeat, reply, 
                               scamsCount >>

RSend(self) == /\ pc[self] = "RSend"
               /\ availSeats' = [availSeats EXCEPT ![self] = reply_[self].seats]
               ...
               /\ UNCHANGED << BankAccount, seatMap, ResOwner, Tickets, CState, 
                               MyTickets, ipOf, ip_, internalReq, id, ip_H, 
                               reply_, target, reservedSeat, doCancel, ip, 
                               targetID, targetSeat, reply, scamsCount >>
...
RWait(self) == /\ pc[self] = "RWait"
               ...
               /\ UNCHANGED << BankAccount, seatMap, ResOwner, Tickets, CState, 
                               MyTickets, ipOf, ip_, internalReq, id, ip_H, 
                               wantSeat, target, reservedSeat, availSeats, ip, 
                               targetID, targetSeat, reply, scamsCount >>
...
PostWait(self) == /\ pc[self] = "PostWait"
                  ...
                  /\ UNCHANGED << BankAccount, seatMap, ResOwner, Tickets, 
                                  ...
                                  availSeats, doCancel, ip, targetID, 
                                  targetSeat, reply, scamsCount >>
...
HClient(self) == InitTarget(self) \/ s1(self) \/ CheckDone(self)
                    ...
                    \/ ChDone(self) \/ LWaitIdle(self) \/ LSend(self)
                    \/ LWait(self) \/ RSend(self) \/ RWait(self)
...
MStep(self) == /\ pc[self] = "MStep"
               ...
               /\ UNCHANGED << BankAccount, Channels, seatMap, ResOwner, 
                               ...
                               reservedSeat, availSeats, doCancel, ip, reply, 
                               scamsCount >>
